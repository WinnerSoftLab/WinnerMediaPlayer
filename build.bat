...
REM $Id: build.bat 4820 2014-04-22 06:57:51Z szl $
REM
REM (C) 2009-2014 see Authors.txt
REM
REM This file is part of WinnerMediaPlayer.
REM
REM WinnerMediaPlayer is free software; you can redistribute it and/or modify
REM it under the terms of the GNU General Public License as published by
REM the Free Software Foundation; either version 3 of the License, or
REM (at your option) any later version.
REM
REM WinnerMediaPlayer is distributed in the hope that it will be useful,
...
FOR %%A IN (%ARG%) DO (
  ...
  IF /I "%%A" == "WinnerMediaPlayer"     SET "CONFIG=MPCBE"      & SET /A ARGC+=1
...
IF /I "%ZIP%" == "True"       CALL :SubCreatePackages WinnerMediaPlayer Win32
...
IF /I "%ZIP%" == "True"       CALL :SubCreatePackages WinnerMediaPlayer x64
...
:End
TITLE Compiling WinnerMediaPlayer [FINISHED]
SET END_TIME=%TIME%
CALL :SubGetDuration
...
:SubFilters
TITLE Compiling WinnerMediaPlayer Filters - %BUILDCFG% Filter^|%1...
...
:SubMPCBE
TITLE Compiling WinnerMediaPlayer - %BUILDCFG%^|%1...
"%MSBUILD%" mpc-be%SLN%.sln %MSBUILD_SWITCHES%^
 /target:%BUILDTYPE% /property:Configuration=%BUILDCFG%;Platform=%1^
 /flp1:LogFile=%LOG_DIR%\WinnerMediaPlayer_errors_%BUILDCFG%_%1.log;errorsonly;Verbosity=diagnostic^
 /flp2:LogFile=%LOG_DIR%\WinnerMediaPlayer_warnings_%BUILDCFG%_%1.log;warningsonly;Verbosity=diagnostic
IF %ERRORLEVEL% NEQ 0 (
...
IF /I "%1" == "Win32" (
  SET "DIR=%BIN%\WinnerMediaPlayer_x86"
) ELSE (
  SET "DIR=%BIN%\WinnerMediaPlayer_x64"
)

IF /I "%SIGN%" == "True" (
  CALL :SubSign %DIR% WinnerMediaPlayer*.exe
  CALL :SubSign %DIR% mpciconlib*.dll
)

TITLE Compiling WMPShellExt - %BUILDCFG%...
"%MSBUILD%" WMPShellExt%SLN%.sln %MSBUILD_SWITCHES%^
 /target:%BUILDTYPE% /property:Configuration=%BUILDCFG%;Platform=Win32
IF %ERRORLEVEL% NEQ 0 (
  CALL :SubMsg "ERROR" "WMPShellExt%SLN%.sln %BUILDCFG% Win32 - Compilation failed!"
) ELSE (
  CALL :SubMsg "INFO" "WMPShellExt%SLN%.sln %BUILDCFG% Win32 compiled successfully"
)

SET "DIR=%BIN%\WinnerMediaPlayer_x86"
IF /I "%SIGN%" == "True" (
  CALL :SubSign %DIR% WMPShellExt.dll
)

"%MSBUILD%" WMPShellExt%SLN%.sln %MSBUILD_SWITCHES%^
 /target:%BUILDTYPE% /property:Configuration=%BUILDCFG%;Platform=x64
IF %ERRORLEVEL% NEQ 0 (
  CALL :SubMsg "ERROR" "WMPShellExt%SLN%.sln %BUILDCFG% x64 - Compilation failed!"
) ELSE (
  CALL :SubMsg "INFO" "WMPShellExt%SLN%.sln %BUILDCFG% x64 compiled successfully"
)

SET "DIR=%BIN%\WinnerMediaPlayer_x64"
IF /I "%SIGN%" == "True" (
  CALL :SubSign %DIR% WMPShellExt64.dll
)
...
IF /I "%1" == "Win32" (
  SET "DIR=%BIN%\WinnerMediaPlayer_x86\Lang"
) ELSE (
  SET "DIR=%BIN%\WinnerMediaPlayer_x64\Lang"
)
...
"%InnoSetupPath%\iscc.exe" /Q /O"%BIN%" "distrib\WinnerMediaPlayer_setup.iss" %ISDefs%
IF %ERRORLEVEL% NEQ 0 CALL :SubMsg "ERROR" "Compilation failed!"
...
IF /I "%SIGN%" == "True" (
  CALL :SubSign %BIN% WinnerMediaPlayer.%MPCBE_VER%.%ARCH%.exe
)
...
IF /I "%~1" == "Filters" (SET "NAME=standalone_filters-WinnerMediaPlayer") ELSE (SET "NAME=WinnerMediaPlayer")
IF /I "%~2" == "Win32" (
  SET ARCH=x86
) ELSE (
  SET ARCH=x64
...
IF /I "%NAME%" == "WinnerMediaPlayer" (
  IF NOT EXIST "%PCKG_NAME%\Lang" MD "%PCKG_NAME%\Lang"
  IF NOT EXIST "%PCKG_NAME%\Shaders" MD "%PCKG_NAME%\Shaders"
  IF /I "%ARCH%" == "x64" (
    COPY /Y /V "%~1_%ARCH%\WinnerMediaPlayer64.exe"                     "%PCKG_NAME%\WinnerMediaPlayer64.exe" >NUL
    COPY /Y /V "%~1_%ARCH%\WMPShellExt64.dll"              "%PCKG_NAME%\WMPShellExt64.dll" >NUL
    ...
    COPY /Y /V "%~1_%ARCH%\WinnerMediaPlayer.exe"                       "%PCKG_NAME%\WinnerMediaPlayer.exe" >NUL
    COPY /Y /V "%~1_%ARCH%\WMPShellExt.dll"                "%PCKG_NAME%\WMPShellExt.dll" >NUL
...
:MissingVar
COLOR 0C
TITLE Compiling WinnerMediaPlayer [ERROR]
