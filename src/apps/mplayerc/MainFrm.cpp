...
void CMainFrame::ShowTrayIcon(bool fShow)
{
	...
	if (fShow) {
		if (!m_fTrayIcon) {
			...
			StringCchCopy(tnid.szTip, _countof(tnid.szTip), TEXT("WinnerMediaPlayer"));
	...
	switch (nIDEvent) {
		...
		case TIMER_STATS: {
			if (m_pQP) {
				...
				WinnerMediaPlayer window should now be hung
				...
				There can be a bug in QUARTZ or more likely WinnerMediaPlayer is doing something wrong
				*/
...
void CMainFrame::SaveThumbnails(LPCTSTR fn)
{
...

	{
		...
		CStringW str;
		str.Format(L"{\\an9\\fs%d\\b1\\bord0\\shad0\\1c&Hffffff&}%s", infoheight-10, width >= 550 ? L"WinnerMediaPlayer" : L"MPC");
...
void CMainFrame::PlayFavoriteFile(CString fav)
{
	...
	//       All you have to do then is plug in your 500 gb drive, full with movies and/or music, start WinnerMediaPlayer (from the 500 gb drive) with a preloaded playlist and press play.
	if (bRelativeDrive) {
		// Get the drive WinnerMediaPlayer is on and apply it to the path list
		CString exePath;
...
{
    // @DeusModus temporary removed updates check
...
{
	ShellExecute(m_hWnd, _T("open"), _T("http://winnermediaplayer.com/"), NULL, NULL, SW_SHOWDEFAULT);
}
...
UINT CMainFrame::YoutubeThreadProc()
{
	...
	HINTERNET f, s = InternetOpen(L"WinnerMediaPlayer Youtube Downloader", 0, NULL, NULL, 0);
	if (s) {
#ifdef _DEBUG
...
afx_msg void CMainFrame::OnLanguage(UINT nID)
{
	...
	if (nID == CMPlayerCApp::GetLanguageIndex(ID_LANGUAGE_HEBREW)) { // Show a warning when switching to Hebrew (must not be translated)
		MessageBox(_T("The Hebrew translation will be correctly displayed (with a right-to-left layout) after restarting the application.\n"),
				   _T("WinnerMediaPlayer"), MB_ICONINFORMATION | MB_OK);
	}
