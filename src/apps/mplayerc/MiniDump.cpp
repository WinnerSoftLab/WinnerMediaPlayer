...
LONG WINAPI CMiniDump::UnhandledExceptionFilter( _EXCEPTION_POINTERS *lpTopLevelExceptionFilter )
{
	...
	if ( hDll != NULL ) {
		...
		if ( pMiniDumpWriteDump != NULL ) {
			if (!AfxGetMyApp()->IsIniValid()) {
				...
				strDumpPath.Append(_T("\\WinnerMediaPlayer\\"));
	...
	if (szResult) {
		//@DeusModus Temporary removed ability to send dumps!
		//if (MessageBox(NULL, szResult, _T("WinnerMediaPlayer Mini Dump"), retval ? MB_YESNO : MB_OK) == IDYES) {
