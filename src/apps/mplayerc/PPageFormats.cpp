...
#ifdef _WIN64
	#define PROGID _T("WinnerMediaPlayer64")
#else
	#define PROGID _T("WinnerMediaPlayer")
#endif // _WIN64
...
bool CPPageFormats::RegisterApp()
{
	...
	if (m_pAAR) {
		...
		if (ERROR_SUCCESS == key.Open(HKEY_LOCAL_MACHINE, _T("SOFTWARE\\RegisteredApplications"))) {
			key.SetStringValue(_T("WinnerMediaPlayer"), GetRegisteredKey());
...
void CPPageFormats::AddAutoPlayToRegistry(autoplay_t ap, bool fRegister)
{
	...
	if (fRegister) {
		...
		key.SetStringValue(_T("Provider"), _T("WinnerMediaPlayer"));
