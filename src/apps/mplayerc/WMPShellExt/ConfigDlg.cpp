...
#define MPCBE_PATH_RU	_T("Текущий путь WinnerMediaPlayer:")
#define MPCBE_PATH_EN	_T("Current WinnerMediaPlayer path:")

#define CANCEL_RU		_T("Отмена")
#define CANCEL_EN		_T("Cancel")

#define CAPTION_RU		_T("Настройки WinnerMediaPlayer ShellExt")
#define CAPTION_EN		_T("WinnerMediaPlayer ShellExt settings")
...
{
	CRegKey key;
	if (ERROR_SUCCESS == key.Create(HKEY_CURRENT_USER, _T("Software\\WinnerMediaPlayer\\ShellExt"))) {
		CString path;
...
BOOL CConfigDlg::OnInitDialog()
{
	...
	if (ERROR_SUCCESS == key.Open(HKEY_LOCAL_MACHINE, _T("Software\\WinnerMediaPlayer"))) {
	...
	if (ERROR_SUCCESS == key.Open(HKEY_LOCAL_MACHINE, _T("Software\\Wow6432Node\\WinnerMediaPlayer"))) {
	...
	CString mpc_path = _T("");
	if (ERROR_SUCCESS == key.Open(HKEY_CURRENT_USER, _T("Software\\WinnerMediaPlayer\\ShellExt"))) {
		len = sizeof(path_buff);
		memset(path_buff, 0, sizeof(path_buff));
