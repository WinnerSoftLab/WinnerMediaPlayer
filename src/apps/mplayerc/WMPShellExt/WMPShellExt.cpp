...
extern "C" __declspec(dllexport) HRESULT DllConfig(LPCTSTR lpszPath)
{
	if (wcslen(lpszPath)) {
		if (::PathFileExists(lpszPath)) {
			CRegKey key;
			if (ERROR_SUCCESS == key.Create(HKEY_CURRENT_USER, L"Software\\WinnerMediaPlayer\\ShellExt")) {
	...
	if (ERROR_SUCCESS == key.Open(HKEY_LOCAL_MACHINE, L"Software\\WinnerMediaPlayer")) {
	...
	if (ERROR_SUCCESS == key.Open(HKEY_LOCAL_MACHINE, L"Software\\Wow6432Node\\WinnerMediaPlayer")) {
...
static CString GetKeyName()
{
	...
	CRegKey key;
	if (ERROR_SUCCESS == key.Open(HKEY_CURRENT_USER, L"Software\\WinnerMediaPlayer\\ShellExt")) {
	...
	if (SUCCEEDED(hr)) {
		...
		if (::StringFromGUID2(CLSID_MPCBEContextMenu, strWideCLSID, 50) > 0) {
			...
			if (GetKeyValue(L"ShowDir")) {
				key.SetValue(HKEY_CLASSES_ROOT, L"directory\\shellex\\ContextMenuHandlers\\WMPShellExt\\", strWideCLSID);
			}
					...
					while ((lRet = reg.EnumKey(dwIndex, szSubKeyName, &cbName)) != ERROR_NO_MORE_ITEMS) {
						if (lRet == ERROR_SUCCESS) {
							...
							if (!key_name.Find(KeyName)) {
								key_name.Append(L"\\shellex\\ContextMenuHandlers\\WMPShellExt\\");
...
STDAPI DllUnregisterServer(void)
{
	...
	if (SUCCEEDED(hr)) {
		if (key.Open(HKEY_CLASSES_ROOT, L"directory\\shellex\\ContextMenuHandlers\\") == ERROR_SUCCESS) {
			key.DeleteSubKey(L"WMPShellExt");
		}
