...
bool LoadType(CString fn, CString& type)
{
	...
	if (!fn.IsEmpty()) {
		...
		// Try WinnerMediaPlayer's internal formats list
...
bool CMPlayerCApp::StoreSettingsToIni()
{
	...
	if (!::PathFileExists(m_pszProfileName)) { // don't overwrite existing ini file
		LPTSTR pszComments = L"; WinnerMediaPlayer";
...
bool CMPlayerCApp::GetAppSavePath(CString& path)
{
	...
	if (IsIniValid()) { // If settings ini file found, store stuff in the same folder as the exe file
		...
		CPath p;
		p.Combine(path, _T("WinnerMediaPlayer"));
		path = (LPCTSTR)p;
	}
...
void CMPlayerCApp::ExportSettings()
{
	...
	CFileDialog fileSaveDialog(FALSE, 0, _T("WinnerMediaPlayer-settings.") + ext,
...
BOOL CMPlayerCApp::InitInstance()
{
	...
	if ((m_s.nCLSwitches&CLSW_UNREGEXT)) { // unregistered file types
		if (IsWinVistaOrLater() && !IsUserAdmin()) {
			...
			BOOL bIs64 = IsW64();
			CPPageFormats::UnRegisterShellExt(GetModulePath(false) + _T("\\WMPShellExt.dll"));
			if (bIs64) {
				CPPageFormats::UnRegisterShellExt(GetModulePath(false) + _T("\\WMPShellExt64.dll"));
			}
	...
	CRegKey key;
	if (ERROR_SUCCESS == key.Create(HKEY_LOCAL_MACHINE, _T("Software\\WinnerMediaPlayer"))) {
		CString path;
		GetModuleFileName(AfxGetInstanceHandle(), path.GetBuffer(_MAX_PATH), _MAX_PATH);
		path.ReleaseBuffer();
	...
	if (!strSatellite.IsEmpty()) {
		...
		if (strSatVersion.GetLength()) {
			...
			if (strSatVersion == strNeededVersion) {
				...
				MessageBox(NULL, _T("Your language pack will not work with this version. Please download a compatible one from the WinnerMediaPlayer homepage."),
						   _T("WinnerMediaPlayer"), MB_OK);
			}
		}
	}
