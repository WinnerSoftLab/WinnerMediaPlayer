...
public:
	CDebugMonitor(DWORD dwProcessId) : CWinDebugMonitor(dwProcessId) {
		...
		m_File = NULL;
		if (bIsInitialize) {
			CString fname = sDesktop + L"WinnerMediaPlayer_debug.log";
			m_File = _tfopen(fname, L"at, ccs=UTF-8");
			if (m_File) {
				fseek(m_File, 0, 2);

				_ftprintf_s(m_File, _T("=== Start WinnerMediaPlayer Debug log [%s] ===\n"), GetLocalTime());
			}
		}
	}
	...
	~CDebugMonitor() {
		if (m_File) {
			_ftprintf_s(m_File, _T("=== End WinnerMediaPlayer Debug log [%s] ===\n"), GetLocalTime());
