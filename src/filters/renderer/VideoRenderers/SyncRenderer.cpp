	...
	m_CurrentAdapter(0)
{
	...
	if (hDll) {
		...
		_Error += L" release or newer in order for WinnerMediaPlayer to function properly.\n";
	}
...
STDMETHODIMP CSyncRenderer::NonDelegatingQueryInterface(REFIID riid, void** ppv)
{
	...
	if (m_pEVR && FAILED(hr)) {
		if (riid == __uuidof(IVMRffdshow9)) { // Support ffdshow queueing. We show ffdshow that this is patched WinnerMediaPlayer.
